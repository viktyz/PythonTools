        ...
        for line in lines:  # 一行行的处理
            ...
            while i < line_length:
                ...
                if self.state == self.c_state.origin.value:  # 初始状态
                    ...
                    if ch in OC_SEPARATOR_LIST:
                        #TODO::
                        self.state = self.c_state.start_sep.value

                elif self.state == self.c_state.start_sep.value:

                    while ch not in OC_SEPARATOR_LIST:
                        string += ch
                        if i < line_length:
                            ch = line[i]
                            i += 1
                            ...
                            break
                    self.state = self.c_state.end_sep.value
                    i -= 1

                elif self.state == self.c_state.end_sep.value:
